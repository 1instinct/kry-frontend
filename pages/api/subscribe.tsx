...
const ghlApiUrl = process.env.NEXT_PUBLIC_GOHIGHLEVEL_API_URL || "";
const ghlApiKey =
  process.env.NEXT_PUBLIC_GOHIGHLEVEL_API_KEY ||
...
async function handleGoHighLevel({
  ...
  const isDebug = process.env.NEXT_PUBLIC_IS_DEBUG_MODE === "development";
  ...
  try {
    ...
    if (searchData && searchData.contacts && searchData.contacts.length > 0) {
      ...
      isDebug && console.log("Contact not found, creating new contact...");
      ...
      if (createResponse.ok) {
        ...
        isDebug && console.log("Failed to create contact");
